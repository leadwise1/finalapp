...
const FreeCareerAI: React.FC = () => {
  const [activeTab, setActiveTab] = useState('resume');
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<{
    resumeAnalysis?: ReturnType<typeof analyzeResumeClientSide>;
    coverLetter?: string;
    interviewPrep?: ReturnType<typeof generateInterviewQuestions>;
  }>({});

  const [error, setError] = useState('');

  // Alternative: Client-side AI analysis (no API needed)
  const analyzeResumeClientSide = (resumeText: string) => {
  ...
  const generateCoverLetterTemplate = (jobTitle: string, companyName: string, userBackground: string) => {
    const templates = {
...
[Your Name]`
  ...
  const generateInterviewQuestions = (jobTitle: string) => {
    const questionBank = {
      general: [
        {
      ...
      tech: [
        ...
        {
          question: "Describe a time you had to debug a difficult problem.",
          tip: "Walk through your systematic approach to problem-solving.",
          category: "Problem Solving"
        }
      ],
      business: [
        {
          ...
          tip: "Describe a systematic approach to prioritization and time management.",
          category: "Organization"
        },
        {
          question: "Tell me about a time you led a team or project.",
          tip: "Focus on your leadership style and the results you achieved.",
          category: "Leadership"
        }
      ]
    };
    ...
    } else if (jobLower.includes('manager') || jobLower.includes('business') || jobLower.includes('analyst')) {
      selectedQuestions = [...selectedQuestions, ...questionBank.business];
    }
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100">
        ...
        <p className="text-sm text-blue-700 mb-4">
          Get instant feedback on your resume using our free AI-powered analysis tool.
        </p>
        ...
        <button
          onClick={() => {
            const resumeText = (document.getElementById('resumeText') as HTMLTextAreaElement).value;
            if (!resumeText.trim()) {
          ...
          disabled={loading}
          className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
        >
      ...
      {results.resumeAnalysis && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            {results.resumeAnalysis.technicalSkillsFound?.length > 0 && (
              <div>
                <h5 className="font-medium text-blue-800 mb-2">Technical Skills Found</h5>
                <div className="flex flex-wrap gap-2">
                  {results.resumeAnalysis.technicalSkillsFound.map((skill, index) => (
            ...
            {results.resumeAnalysis.softSkillsFound?.length > 0 && (
              <div>
                <h5 className="font-medium text-purple-800 mb-2">Soft Skills Found</h5>
                <div className="flex flex-wrap gap-2">
                  {results.resumeAnalysis.softSkillsFound.map((skill, index) => (
          ...
          <div className="mb-6">
            <h5 className="font-medium text-gray-800 mb-2">Suggested Keywords to Add</h5>
            <div className="flex flex-wrap gap-2">
              {results.resumeAnalysis.keywordsSuggested.map((keyword, index) => (
                <span key={index} className="bg-gray-100 text-gray-700 px-2 py-1 rounded-full text-xs border border-gray-300">
                  {keyword}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {results.resumeAnalysis.recommendations.map((recommendation, index) => (
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-green-50 to-teal-50 p-6 rounded-xl border border-green-100">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-xl font-semibold text-gray-800">AI Cover Letter Generator</h3>
          <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium">FREE</span>
        </div>
        <p className="text-sm text-green-700 mb-6">
          Generate personalized cover letters instantly using our smart templates.
        </p>
        ...
        <button
          onClick={() => {
            ...
            const userBackground = (document.getElementById('userBackground') as HTMLTextAreaElement).value;
            ...
            setTimeout(() => {
              ...
              setResults(prev => ({ ...prev, coverLetter }));
              setLoading(false);
            }, 1000);
          }}
          disabled={loading}
          className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
        >
      ...
      {results.coverLetter && (
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => results.coverLetter && navigator.clipboard.writeText(results.coverLetter)}
          ...
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-800 text-sm">
              <strong>Pro Tip:</strong> Customize this template further by adding specific achievements, 
              company research details, and personal touches to make it uniquely yours.
            </p>
          </div>
        </div>
      )}
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-100">
        ...
        <p className="text-sm text-purple-700 mb-6">
          Get ready for your interview with tailored questions and expert tips.
        </p>
        ...
        <button
          onClick={() => {
            const jobTitle = (document.getElementById('interviewJobTitle') as HTMLInputElement).value || 'General Position';
            setLoading(true);
            
            setTimeout(() => {
              const interviewPrep = generateInterviewQuestions(jobTitle);
              setResults(prev => ({ ...prev, interviewPrep }));
              setLoading(false);
            }, 800);
          }}
          disabled={loading}
          className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
        >
      ...
      {results.interviewPrep && (
        <div className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Common Interview Questions</h4>
            <div className="space-y-4">
              {results.interviewPrep.questions.map((q, index) => (
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Interview Tips</h4>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <h5 className="font-medium text-blue-800 mb-2">Preparation Tips</h5>
                <ul className="space-y-2">
                  {results.interviewPrep.tips.map((tip, index) => (
              ...
              <div>
                <h5 className="font-medium text-purple-800 mb-2">Questions to Ask Them</h5>
                <ul className="space-y-2">
                  {results.interviewPrep.questionsToAsk.map((question, index) => (
                    <li key={index} className="text-sm text-gray-700 flex items-start gap-2">
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mt-12 pt-8 border-t border-gray-200">
          <p className="text-gray-500 text-sm">
            Built for underrepresented individuals • 100% Free • No API keys required
          </p>
        </div>
      </div>
    </div>
